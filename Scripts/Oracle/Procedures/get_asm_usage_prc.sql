...
grant select on v_$instance to maint;
grant select on v_$asm_disk to maint;
grant select on v_$asm_diskgroup_stat to maint;

create or replace procedure maint.get_asm_usage_prc as
  /*
    NAME
  ...
  if l_gr_full_cnt = 0 then
    ...
    for group_info in (select name as asm_group,
      ...
      l_tech_owner_alert := 'Дисковая группа ' || group_info.asm_group ||
                            ...
                            ' Гб. С учётом размера минимального размера диска в группе, а так же типа избыточности, необходимо добавить диск(и) в количестве ' ||
